...
object NfcStateCondition : CapabilityDefinition {
    override val type = "android.nfc.state"
    override val lane = CapabilityLane.CONDITION
    override val displayName = "NFC state"
    override val description = "Checks whether NFC is enabled or disabled."
            ...
            if (block.expectedNfcEnabledOrNull() == null) {
                add(
                    ValidationIssue(
                        ...
                        code = "invalid_nfc_state",
                        message = "NFC state must be 'enabled' or 'disabled'.",
                    ),
                )
            }
        }
    ...
    override fun explain(block: MacroBlock): String =
        if (block.expectedNfcEnabledOrNull() == false) {
            "Continue only while NFC is disabled."
        } else {
            "Continue only while NFC is enabled."
        }

    override fun requiredPermissions(block: MacroBlock): Set<AndroidPermission> = emptySet()

    override fun compile(block: MacroBlock): RuntimeStep =
        RuntimeStep.CheckNfcEnabled(
            blockId = block.id,
            expectedEnabled = requireNotNull(block.expectedNfcEnabledOrNull()),
        )
}

internal fun MacroBlock.expectedNfcEnabledOrNull(): Boolean? =
